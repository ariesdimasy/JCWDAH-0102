...
function buy(money: number, product: IProduct, callback?: (leftMoney: number) => void) {
    let leftMoney = money - product.price
...
buy(5000000, listProduct[1], (leftMoney) => {
    let res = leftMoney - listProduct[0].price
    console.log("You buy ", listProduct[0].name, " price ", listProduct[0].price, ".")
    console.log("your left money is ", res)
    buy(res, listProduct[3], (leftMoney) => {
        let res = leftMoney - listProduct[3].price
